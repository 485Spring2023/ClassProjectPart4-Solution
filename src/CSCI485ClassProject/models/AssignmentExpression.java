...
public class AssignmentExpression {
  ...
  // returns the Map, from the modified attributeName to the attr's new value
  public Map<String, Object> evaluate(Record record) {
    if (record == null) {
      return null;
    }
    ...
    if (expressionType == Type.ONE_ATTR) {
      res.put(leftHandSideAttrName, rightHandSideValue);
    } else {
      ...
      if (leftHandSideAttrType == AttributeType.INT) {
        resultVal = AlgebraUtils.computeTwoINT(rightVal, rightHandSideValue, rightHandSideOperator);
      } else {
        resultVal = AlgebraUtils.computeTwoDOUBLE(rightVal, rightHandSideValue, rightHandSideOperator);
      }
      res.put(leftHandSideAttrName, resultVal);
    }
